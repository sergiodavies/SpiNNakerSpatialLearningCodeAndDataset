...
int main()
{
    FILE *csvSource = fopen("3values_to_test.csv", "r");
    FILE *batchOutput = fopen("batch_3_values.sh", "w");
    ...
    for(;;)
    {
        ...
        ret = fscanf(csvSource, "%*d,%*d,%*d,%d,%d,%d\n", &cw0, &cw1, &cw2);

        if (ret != 3)
            break;

        snprintf (foldername, 256, "saved_data/Multiple_patterns_%d+%d+%d", cw0, cw1, cw2);
        fprintf (batchOutput, "mkdir -p %s\n", foldername);
        fprintf (batchOutput, "ipython train_bit_pattern.py << EOF\n");
        fprintf (batchOutput, "%d %d %d\n", cw0, cw1, cw2);
